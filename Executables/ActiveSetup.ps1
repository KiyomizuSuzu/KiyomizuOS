...
# ============================================================================
try {
    $lightTheme = (Get-ItemProperty `
                    -Path 'HKCU:\Software\AME' `
                    -ErrorAction Stop).UseLightTheme
...
$wallpaperPath = if ($lightTheme -eq 1) {
...
elseif (-not $Phase2) {
    ...
    Set-RegistryValue -Path "Registry::HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize" `
        ...
        -Type 'DWord' `
        -Value 0 `
        -Desc 'Choose between Light and Dark mode for Windows and apps'
