...
# CONFIGURATION
# ============================================================================
$username = (((quser) -match '^>') -replace '^>', '').Trim().Split()[0]
$sid = ([System.Security.Principal.NTAccount]"$env:COMPUTERNAME\$username").Translate([System.Security.Principal.SecurityIdentifier]).Value
...
# EXECUTION
# ============================================================================
Set-BinaryBit -Path "Registry::HKEY_USERS\$sid\Control Panel\Desktop" `
    -Name 'UserPreferencesMask' `
    -ByteIndex 4 `
    -BitMask 0x02 `
    -SetBit $False `
    -Desc 'System UI animations'
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Control Panel\Desktop\WindowMetrics" `
    -Name 'MinAnimate' `
    -Type 'String' `
    -Value 0 `
    -Desc 'Window minimize/maximize animation'
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" `
    -Name 'TaskbarAnimations' `
    -Type 'DWord' `
    -Value 0 `
    -Desc 'Taskbar animations'
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Software\Microsoft\Windows\DWM" `
    -Name 'EnableAeroPeek' `
    -Type 'DWord' `
    -Value 1 `
...
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Control Panel\Desktop" `
...
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Software\Microsoft\Windows\CurrentVersion\Search" `
    -Name 'SearchboxTaskbarMode' `
    -Type 'DWord' `
    -Value 1 `
    -Desc 'Choose how Windows search appears on the taskbar: hidden, icon only, icon with label, or full search box'
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Software\Classes\CLSID\{86ca1aa0-34aa-4e8b-a509-50c905bae2a2}\InprocServer32" `
    -Name '(Default)' `
    -Type 'String' `
    -Value '' `
    -Desc 'Use the Windows 10-style right-click menu with all options visible instead of the simplified Windows 11 menu'
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Control Panel\Accessibility\HighContrast" `
    -Name 'Flags' `
    -Type 'String' `
    -Value 4194 `
    -Desc 'High contrast shortcut'
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Software\Classes\CLSID\{f874310e-b6b7-47dc-bc84-b9e6b38f5903}" `
    -Name 'System.IsPinnedToNameSpaceTree' `
    -Type 'DWord' `
    -Value 0 `
    -Desc 'Show Home folder'
Set-RegistryValue -Path "Registry::HKEY_USERS\$sid\Software\Classes\CLSID\{e88865ea-0e1c-4e20-9aa6-edcd0212c87c}" `
    -Name 'System.IsPinnedToNameSpaceTree' `
    -Type 'DWord' `
    -Value 0 `
